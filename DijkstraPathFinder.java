...
/*
 * Dev Notes:
    * Implement Dijkstra's with baritone;
    ...
    *
    * Have to update everything the original algorithm updates so the player actually moves
    * We need to keep track of the lowest cost edge probably going to do it with Baritone's Binary Heap
    * Need to use a couple of data structures from baritone like the node and binary heap
    * For each node/block in minecraft baritone has a specific hash to identfy each one
    * Also need to update and feed-back a Path object that baritone uses to move the player in minecraft
    *
 ...
 * 
 * Add node to binary heap then update cost for every neighbor
...
public final class DijkstraPathFinder extends AbstractNodeCostSearch {
    ...
    private final CalculationContext calcContext; // Added for Baritone to work
    private static final int timeCheckInterval = 1000000; //to log the times
    private boolean failing = false; //for if the path fails
    ...
    } //creats a txt file for logging times

    @Override // Main method to calculate the path
